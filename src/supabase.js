...
// Embed category name + tags so pills/amenities can filter on them.
export async function fetchEvents({ freeOnly = false } = {}) {
  const select = '*,categories(name),event_tags(tags(name,tag_group))'
...
export function mapEvent(e) {
  ...
  return {
    ...
    // Read event_type (schema) first, fall back to content_type (old front end)
